...
import com.junda.springbootmall.dto.UserLoginRequest;
...
public class UserServicelmpl implements UserService {
    ...
    private final static Logger log = LoggerFactory.getLogger(UserServicelmpl.class);
    ...
    public User login(UserLoginRequest userLoginRequest) {
        ...
        // 使用 MD5 生成密碼雜湊值
        String hasdedPassword = DigestUtils.md5DigestAsHex(userLoginRequest.getPassword().getBytes());


        // 比較密碼
        if (user.getPassword().equals(hasdedPassword)) {
            return user;
        } else {
